...
async def analyser(request: Request):
    ...
    if r.get("ou_securite"):
        ...
        <div class="box" style="border:2px solid #60a5fa;">
            ...
            <p style="font-size:11px;color:#888;margin-top:6px;">Filet de sécurité. Stake suggéré : 0.5% bankroll.</p>
        </div>
        """
    ...
    if r.get("divergences"):
        ...
        for d in r["divergences"]:
            if d["niveau"] == "MAJEUR":
                bord = "#ef4444"; color = "#ef4444"
            else:
                bord = "#eab308"; color = "#eab308"
            ecart_pct = d["ecart"] * 100
            blocs += f"""
            <div class="box" style="border-left:4px solid {bord};padding-left:10px;">
                ...
                <div class="ligne"><span class="label">Écart</span><span class="val" style="color:{color};">{ecart_pct:+.1f}%</span></div>
            ...
            """
        divergences_html = f"""
        <h2 style="text-align:left;">🔍 Divergences détectées</h2>
        <div class="box" style="background:#1a1a26;border-color:#3a3a4e;">
            <p style="font-size:12px;color:#aaa;margin:0 0 8px 0;">Comparaison Modèle (Poisson) vs Marché (cote implicite démargée).</p>
        </div>
        {blocs}
        """
    else:
        divergences_html = """
        <h2 style="text-align:left;">🔍 Divergences détectées</h2>
        <div class="box"><p style="color:#4ade80;font-size:13px;">✅ Aucune divergence majeure — modèle et marché sont alignés.</p></div>
        """
