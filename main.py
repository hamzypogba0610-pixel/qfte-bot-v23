...
async def analyser(request: Request):
    ...
    html += '<p style="text-align:center;color:#999;font-size:12px;">' + eq_dom + ' vs ' + eq_ext + ' - ' + competition + '</p>'

    # DECISION
    if r["pari_retenu"]:
        p = r["pari_retenu"]
        html += '<div class="box" style="border:2px solid #4ade80;">'
        html += '<h2 style="color:#4ade80;">PARI RETENU (' + p.get("type", "1X2") + ')</h2>'
    ...
    html += '<div class="ligne"><span class="label">Ratio HT dom/ext</span><span class="val">' + str(d["ratio_ht_home"]) + ' / ' + str(d["ratio_ht_away"]) + '</span></div>'
    ...
    html += '<div class="ligne"><span class="label">Facteur class.</span><span class="val">' + str(d["f_class_home"]) + ' / ' + str(d["f_class_away"]) + '</span></div>'
